...
func ResponseStringWriter(w http.ResponseWriter, messages []string) error {
	var msg string
...
func handlePayment(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		err = ResponseStringWriter(w, []string{"during reading body error", err.Error()})
		if err != nil {
	...
	if err != nil {
		...
		err = ResponseStringWriter(w, []string{"during parsing payment amount occurred error, maybe payment amount is not integer.", err.Error()})
		if err != nil {
			logger.Ptc("fail to write HTTP response: ", err)
		}
		return
	...
	if money-paymentAmount < 0 {
		err := ResponseStringWriter(w, []string{"insufficient funds. balance:", strconv.Itoa(money)})
		if err != nil {
			logger.Ptc("fail to write HTTP response: ", err)
		}
		return
	...
	money -= paymentAmount
	err = ResponseStringWriter(w, []string{"payment is success. balance:", strconv.Itoa(money)})
	if err != nil {
		logger.Ptc("fail to write HTTP response: ", err)
	}
}
...
func handleSaveBalance(w http.ResponseWriter, r *http.Request) {
	...
	if money-saveAmount < 0 {
		err := ResponseStringWriter(w, []string{"insufficient funds for save balance. balance:", strconv.Itoa(money), "bank:", strconv.Itoa(bank)})
		if err != nil {
			logger.Ptc("fail to write HTTP response: ", err)
		}
		return
	...
	money -= saveAmount
	bank += saveAmount
	err = ResponseStringWriter(w, []string{"money:", strconv.Itoa(money), "bank:", strconv.Itoa(bank)})
	if err != nil {
		logger.Ptc("fail to write HTTP response: ", err)
	}
	return
